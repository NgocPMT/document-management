CREATE TYPE "public"."document_status" AS ENUM('UPLOADING', 'READY', 'PROCESSING', 'FAILED');--> statement-breakpoint
...
ALTER TABLE "document_shares" ADD CONSTRAINT "document_shares_user_id_user_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."user"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "document_summary" ADD CONSTRAINT "document_summary_document_id_documents_id_fk" FOREIGN KEY ("document_id") REFERENCES "public"."documents"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
